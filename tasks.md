...
- [ ] **Block-based Editor**: Transition to a rich text block editor (e.g., TipTap or Editor.js) to support real-time formatting, block dragging/dropping, and slash commands (`/`).
...
- [ ] **Automated Code Quality Pipeline**: Introduce GitHub Actions to automatically run linting (ESLint, Prettier) and type-checking on every Push/PR.
- [ ] **Backend Testing**: Add comprehensive unit testing for the FastAPI backend using `pytest`.
- [ ] **Frontend Testing**: Add component tests with `Vitest` and basic End-to-End user flow tests with `Playwright`.
- [ ] **Automated Tauri Releases**: Configure GitHub Actions to automatically compile, sign, and release desktop binaries (.exe, .dmg, .AppImage) when a new version tag is pushed.
- [ ] **Dependency Management**: Set up Dependabot or Renovate to keep frontend packages and Python requirements continuously updated.
